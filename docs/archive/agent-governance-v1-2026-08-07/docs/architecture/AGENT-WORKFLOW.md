...
- **Owner**：只实现任务的功能要求或非功能要求、验收标准和已批准技术设计明确要求的内容，并提供证据。
...
开发 Agent 只能实现功能要求或非功能要求、验收标准和已批准技术设计明确列出的内容。发现新需求、未批准或超出已批准范围的 API 契约/数据模型变化、超出允许修改范围、新增第三方依赖、影响其他模块或验收标准无法覆盖的新行为时，必须停止相关开发并转 `BLOCKED`（不受影响且不接触同一锁的工作可继续）。
...
每个实际实现，包括代码、页面、API、数据表或字段、第三方依赖、基础设施和抽象层，都必须同时映射到 `Requirement Source`、功能要求或非功能要求，以及验收标准；无法映射的内容不得实现。纯安全、性能、兼容性、合规或可维护性实现可以只映射非功能要求，但仍必须映射 `Requirement Source` 和验收标准。每个任务必须明确最小实现范围、明确不实现范围和推迟到未来的内容。未来需求只能进入 backlog，不得提前进入当前实现。
...
发现新增未批准功能、新增未批准依赖（含第三方依赖）、修改未获批准的 API 契约或超出任务已批准 API 修改范围、修改未获批准的数据模型或超出任务已批准数据模型修改范围、增加未批准基础设施、增加未要求的页面或接口、无关重构、为未来需求提前抽象，或修改超出任务允许范围时，必须停止相关工作，并依第 7 节发起 Change Request；未经批准不得恢复。

API 契约或数据模型变化同时满足以下条件时，无需再次发起 Change Request：已写入当前批准的任务基线；已写入允许修改范围或复杂度预算；已映射到 `Requirement Source`；已映射到功能要求或非功能要求；已映射到验收标准；实际实现没有超出已批准范围。只有新发现、未批准或超出批准范围的变化才必须停工并依第 7 节处理，不建立第二套审批流程。
