...
- Module Lock：HANDED_OFF（9 项实施锁全部 HANDED_OFF；READY_FOR_RETEST 保持 HANDED_OFF；等待 Codex Reviewer 第四次复审 RV-001/RV-002）
...
9. **coverlet.collector 不存在**：按 AC-SC-13 验证原则检查（仅检查项目源文件，禁止扫描 bin/obj/DLL/PDB/TestResults）。脚本使用 `grep -ri "coverlet" --exclude-dir=bin --exclude-dir=obj tests/`。退出码 1（无匹配）为 PASS；退出码 0（命中）或 ≥2（执行错误）为 FAIL。已废除 `grep -r "coverlet" tests/`（无排除递归扫描，见 AC-SC-13 废除命令列表）。
...
- 第四次复审结果：0 BLOCKER / 2 MAJOR / 0 MINOR（RV-001/RV-002）
- 已确认关闭：IR-001 CLOSED、IR-002 CLOSED、IR-003/RT-001 CLOSED、IR-004/RT-002 CLOSED、RT-003 SPEC_CLARIFIED
- 本次修复：RV-001（AC-SC-17 gate 9 引用 AC-SC-13 方法，废除旧递归 grep）、RV-002（记录 CHANGES_REQUESTED → IN_FIX → READY_FOR_RETEST 状态迁移）
- 当前任务状态：READY_FOR_RETEST，待 Codex Reviewer 执行第四次（终审）复审
...
| RV-001 | MAJOR | 第四次复审报告 §7；`TASK-0006` AC-SC-17 gate 9 | AC-SC-17 gate 9 改为引用 AC-SC-13 验证原则，使用 --exclude-dir=bin --exclude-dir=obj，明确退出码语义 | CLOSED（AC-SC-17 gate 9 已更新；一致于 AC-SC-13 和 verify-project.ps1 实现） |
| RV-002 | MAJOR | 第四次复审报告 §7；`tasks/current-task.md`、`tasks/TASK-0006-PROJECT-SCAFFOLD.md` | 记录 CHANGES_REQUESTED → IN_FIX → READY_FOR_RETEST 状态迁移与锁交接 | CLOSED（两次迁移已记录；状态、锁、交接记录一致） |
...
| RV-001 | Cursor Developer | AC-SC-17 gate 9 引用 AC-SC-13 验证原则；使用 --exclude-dir=bin --exclude-dir=obj；废除旧递归 grep | Coverlet 验证 PASS（XML exit 0；grep exit 1）；AC-SC-13/AC-SC-17/verify-project.ps1 三者一致 | 待本轮提交 |
| RV-002 | Cursor Developer | 记录 CHANGES_REQUESTED → IN_FIX → READY_FOR_RETEST；同步状态/锁/交接记录 | 工作流 20/20；git diff --check PASS；状态一致 | 待本轮提交 |
...
- 最终 Reviewer：Codex Reviewer（待第四次复审）
- 第三次复审结论：NEEDS_CHANGES（RETEST-2，RT-003 SPEC_CLARIFIED）
- 第四次复审结论：NEEDS_CHANGES（RETEST-3，RV-001/RV-002，见本修复记录）
- RT-003（MAJOR）：SPEC_CLARIFIED — AC-SC-13 改为结构化 XML + 精确 grep；verify-project.ps1 无需修改
- RV-001（MAJOR）：已修复 — AC-SC-17 gate 9 引用 AC-SC-13 方法，使用 --exclude-dir=bin --exclude-dir=obj，一致于 AC-SC-13 和 verify-project.ps1
- RV-002（MAJOR）：已修复 — CHANGES_REQUESTED → IN_FIX → READY_FOR_RETEST 迁移已记录；状态/锁/交接记录一致
- 下一步：Codex Reviewer 执行第四次（终审）READY_FOR_RETEST 复审
