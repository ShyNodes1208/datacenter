...
- [ ] AC-ARCH-01：架构覆盖全部 12 项 FR。文档第 21 节追踪关系表将每项 FR 映射到至少一个架构落脚点。
- [ ] AC-ARCH-02：架构覆盖全部 7 项 NFR。文档第 21 节追踪关系表将每项 NFR 映射到至少一个架构落脚点。
- [ ] AC-ARCH-03：架构未修改产品基线的任何 BR、FR、NFR 或 AC 的业务含义。
- [ ] AC-ARCH-04：文档第 7 节和第 8 节明确区分前端职责和后端职责，BR 校验归属明确。
- [ ] AC-ARCH-05：文档第 9 节和第 10 节确定 SQLite 为数据存储方案，6 个实体职责和关系清晰，选择理由和排除项完整。
- [ ] AC-ARCH-06：文档第 13 节给出前端启动、后端启动、构建和测试的可执行命令及工作目录。
- [ ] AC-ARCH-07：文档第 14 节定义后端单元测试、后端集成测试和前端测试的框架与覆盖范围，至少 12 条核心 BR 被列出为必测项。
- [ ] AC-ARCH-08：文档第 16 节与产品基线 NFR-007、AC-020、AC-037 的角色矩阵一致，匿名访问全页面拒绝。
- [ ] AC-ARCH-09：文档第 17 节未重新定义产品基线的数据规模、硬件边界、网络条件、计时范围、重复次数、统计口径或 3 秒阈值，仅描述执行方式。
- [ ] AC-ARCH-10：文档第 19 节明确列出不实现的功能、架构能力和运维能力。
- [ ] AC-ARCH-11：文档第 18 节列出所有已批准的运行时依赖和明确不引入的依赖类型，每个依赖有理由。
- [ ] AC-ARCH-12：文档第 22 节将后续开发拆分为至少 8 个独立任务，每个任务有范围摘要、前置依赖和预计修改目录。
- [ ] AC-ARCH-13：未包含任何业务代码文件、数据库迁移文件、API Controller、Vue 页面或项目脚手架。
- [ ] AC-ARCH-14：未出现 MVP 范围扩张（如 3D、实时监控、微服务、消息队列、完整 RBAC、CMDB 同步等）。
- [ ] AC-ARCH-15：工作流校验 20/20 PASS、退出码 0；`git diff --check` PASS。
...
- 提交说明：docs: define task-0005 mvp architecture baseline
- 提交哈希：待提交
- 推送结果：待推送
- 本地哈希：待提交
- 远端哈希：待推送
...
- API 风格（Minimal API 或 Controller）由 TASK-0007 决定。

## 最终完成条件

- [ ] 独立 Reviewer 验收或复审通过
- [ ] 验收标准全部通过（AC-ARCH-01 至 AC-ARCH-15）
- [ ] 所有缺陷关闭
- [ ] 构建和测试通过或有批准的 N/A
